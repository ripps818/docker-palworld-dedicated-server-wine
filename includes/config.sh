...
ENVSUBST_SELECTORS='$DIFFICULTY $RANDOMIZER_TYPE $RANDOMIZER_SEED $IS_RANDOMIZER_PAL_LEVEL_RANDOM 
    ...
    $ALLOW_CLIENT_MOD $SHOW_JOIN_LEFT_MESSAGE $ALLOW_ENHANCE_STAT_ATTACK 
    $ALLOW_ENHANCE_STAT_HEALTH $ALLOW_ENHANCE_STAT_STAMINA $ALLOW_ENHANCE_STAT_WEIGHT 
    $ALLOW_ENHANCE_STAT_WORK_SPEED $DISPLAY_PVP_ITEM_NUM_ON_WORLD_MAP_BASE_CAMP 
    $DISPLAY_PVP_ITEM_NUM_ON_WORLD_MAP_PLAYER $ENABLE_FAST_TRAVEL_ONLY_BASE_CAMP 
...
function log_settings_with_counter() {
    ...
    for selector in ${ENVSUBST_SELECTORS}; do
        ...
        if [[ " ${PANEL_OVERRIDDEN_KEYS} " == *" ${varname} "* ]]; then
            e_with_counter "${varname} to '${!varname}' (panel override)"
        else
...
function apply_server_name_token() {
    ...
    if [[ -f "$token_file" ]]; then
        rand="$(trim < "$token_file" 2>/dev/null || true)"
    fi
    ...
    if [[ ! "$rand" =~ ^[A-Za-z0-9]{1,16}$ ]]; then
        rand="$(LC_ALL=C tr -dc 'A-Za-z0-9' </dev/urandom | head -c 6 || true)"
...
function apply_settings_overrides() {
    ...
    while IFS= read -r line || [[ -n "$line" ]]; do
        ...
        key="${line%%=*}"
        value="${line#*=}"
        # Strip matching outer double or single quotes if present
        if [[ "$value" =~ ^\"(.*)\"$ ]] || [[ "$value" =~ ^\'(.*)\'$ ]]; then
